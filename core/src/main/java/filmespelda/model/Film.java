...
public class Film {
    ...
    /**
     * List mert fontos a sorrend(Főszereplők)
     */
    ...
    public void setMegjelenes_datuma(LocalDate megjelenes_datuma) throws Unborn {
        if (megjelenes_datuma.isBefore(LocalDate.now().plusMonths(3)))
            throw new Unborn(megjelenes_datuma.toString());
    ...
    public Film(int hossz, String magyar_cim, String eredeti_cim, List<Szereplo> szereplok, Collection<Rendezo> rendezok, Collection<Mufaj> mufajok, LocalDate megjelenes_datuma, Besorolas besorolas, double imdb_pont) throws Unborn, InvalidValue {
        setHossz(hossz);
        this.magyar_cim = magyar_cim;
        this.eredeti_cim = eredeti_cim;
        this.szereplok = szereplok;
